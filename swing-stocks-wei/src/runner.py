"""Entry point: load data, compute signals, simulate portfolio, persist results."""
...
def main() -> None:
    ...
    log.info("run_label=%s window=%s..%s EMA%d/%d stress %s/%s "
             "weights deep/mild/pos %s/%s/%s%% max %d pos (%d/cat) cost %sbps",
             cfg.run_label, cfg.start_date, cfg.end_date,
    ...
    try:
        ...
        result = run_portfolio(
            ...
            closes=closes[start:], fresh=fresh[start:], positions=positions[start:],
            stress_on=stress[start:], cat_momentum=cat_mom_eval,
            weight_pct_by_tier={"deep": cfg.weight_deep_pct,
                                "mild": cfg.weight_mild_pct,
                                "pos": cfg.weight_pos_pct},
            deep_threshold=cfg.cat_mom_deep_threshold,
            max_positions=cfg.max_positions, max_per_category=cfg.max_per_category,
            cost_bps_per_side=cfg.cost_bps_per_side,
        )

        run_id = persist_run(conn, cfg, result, len(symbols),
                             lagged[start:], stress[start:])

        log.info("=== run_id=%d %s..%s universe=%d ===",
        ...
        log.info("trades: %d (%d winners, %d still open)", len(result.trades),
        ...
        for tier, rets in sorted(by_tier.items()):
            log.info("  tier %-4s n=%3d  avg %+6.2f%%  win %4.1f%%", tier, len(rets),
                     float(np.mean(rets)),
