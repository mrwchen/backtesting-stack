...
def compute_category_breadth(
    close: pd.DataFrame,
    universe: pd.DataFrame,
    cfg: Config,
) -> dict[str, pd.DataFrame]:
    """Return per-symbol daily IBKR category breadth and pass flags."""
    ...
    if not cfg.ibkr_category_breadth_filter_enable:
        pass_by_symbol = pd.DataFrame(True, index=close.index, columns=symbols)

    return {
        "ibkr_category_breadth": breadth_by_symbol,
        "ibkr_category_breadth_on": on_by_symbol,
        "ibkr_category_breadth_pass": pass_by_symbol,
    }
