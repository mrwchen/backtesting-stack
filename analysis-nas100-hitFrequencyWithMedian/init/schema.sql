...
DROP TABLE IF EXISTS backtest2_nas100_hfmed_range_daily_session_stats_for_grafana CASCADE;
...
CREATE TABLE IF NOT EXISTS backtest2_nas100_hfmed_range_analysis (
    ...
    range_to_price_pct              NUMERIC(18,8),
    range_to_price_bps              NUMERIC(18,6),
...
CREATE TABLE IF NOT EXISTS backtest2_nas100_hfmed_range_daily_session_stats_for_grafana (
    ...
    day_first_cross_ts             TIMESTAMPTZ    NOT NULL,
    day_last_cross_ts              TIMESTAMPTZ    NOT NULL,
    min_range_to_price_bps         NUMERIC(18,6),
    avg_range_to_price_bps         NUMERIC(18,6),
    median_range_to_price_bps      NUMERIC(18,6),
    p75_range_to_price_bps         NUMERIC(18,6),
    p95_range_to_price_bps         NUMERIC(18,6),
    max_range_to_price_bps         NUMERIC(18,6),

    PRIMARY KEY (day_start_ts, analysis_id, session_sort_order, lookback_bars)
);

SELECT create_hypertable(
    'backtest2_nas100_hfmed_range_daily_session_stats_for_grafana',
    'day_start_ts',
    chunk_time_interval => :'hfmed_range_analysis_chunk_interval'::interval,
    if_not_exists => TRUE
);

CREATE INDEX IF NOT EXISTS backtest2_nas100_hfmed_range_daily_session_stats_lookup_idx
    ON backtest2_nas100_hfmed_range_daily_session_stats_for_grafana(symbol, session_sort_order, lookback_bars, day_start_ts DESC);

CREATE INDEX IF NOT EXISTS backtest2_nas100_hfmed_range_daily_session_stats_run_idx
    ON backtest2_nas100_hfmed_range_daily_session_stats_for_grafana(analysis_id, session_sort_order, lookback_bars, day_start_ts);

GRANT SELECT, INSERT, UPDATE, DELETE ON TABLE backtest2_nas100_hfmed_range_daily_session_stats_for_grafana TO "market-data-account";
