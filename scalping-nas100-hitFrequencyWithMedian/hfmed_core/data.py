"""Load Pepperstone ticks into compact arrays and derive mid-price bars."""
...
from .sessions import classify_timestamp_codes
...
class TickData:
    ...
    def slice_indices(self, start: int, end: int) -> "TickData":
        return TickData(
            tick_time_ns=self.tick_time_ns[start:end],
            bid=self.bid[start:end],
            ask=self.ask[start:end],
    ...
    def slice_time(self, start_ns: int, end_ns: int) -> "TickData":
        start = int(np.searchsorted(self.tick_time_ns, start_ns, side="left"))
        end = int(np.searchsorted(self.tick_time_ns, end_ns, side="left"))
        return self.slice_indices(start, end)
...
class BarData:
    ...
    def slice_indices(self, start: int, end: int) -> "BarData":
        return BarData(
            bar_start_ns=self.bar_start_ns[start:end],
            open=self.open[start:end],
            high=self.high[start:end],
            low=self.low[start:end],
            close=self.close[start:end],
            tick_count=self.tick_count[start:end],
        )

    def slice_time(self, start_ns: int, end_ns: int) -> tuple["BarData", int, int]:
        start = int(np.searchsorted(self.bar_start_ns, start_ns, side="left"))
        end = int(np.searchsorted(self.bar_start_ns, end_ns, side="left"))
        return self.slice_indices(start, end), start, end
...
def load_ticks(conn: psycopg2.extensions.connection, cfg: RunConfig) -> RawTickData:
    ...
    try:
        with conn.cursor(name="hfmed_tick_loader") as cur:
    ...
    log.info(
        ...
        invalid_rows,
        cfg.symbol,
        _fmt_ns(tick_time_ns[0]),
        _fmt_ns(tick_time_ns[-1]),
    )
    return RawTickData(
...
def build_mid_bars(raw_ticks: RawTickData, cfg: RunConfig) -> tuple[TickData, BarData]:
    if len(raw_ticks) <= 0:
...
def _fmt_ns(value: int) -> str:
    return ns_to_datetime(int(value)).isoformat()
