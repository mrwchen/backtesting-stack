...
PARAMETER_NAMES = [
    ...
    "ALL_STOP_MODES_TAKE_PROFIT_ATR_MULT",
    "BAND_STOP_MIN_PROFILE_RANGE_ATR_MULT",
    "BAND_STOP_PROFILE_LOWER_QUANTILE",
    "BAND_STOP_PROFILE_UPPER_QUANTILE",
    "BAND_STOP_PROFILE_BUFFER_POINTS",
    "BAND_STOP_MIN_DISTANCE_ATR_MULT",
    "BAND_STOP_MAX_DISTANCE_ATR_MULT",
]
...
def _non_profile_combinations(grid: dict[str, list[int | float]]) -> list[tuple[int | float, ...]]:
    ...
    for values in itertools.product(*(grid[name] for name in NON_PROFILE_PARAMETER_NAMES)):
        ...
        min_stop = float(candidate["BAND_STOP_MIN_DISTANCE_ATR_MULT"])
        max_stop = float(candidate["BAND_STOP_MAX_DISTANCE_ATR_MULT"])
...
def is_valid(values: dict[str, int | float]) -> bool:
    try:
        ...
        if float(values["ENTRY_PRICE_RANGE_POSITION_MAX_DEVIATION_PCT"]) < 0:
            return False
        if float(values["ALL_STOP_MODES_TAKE_PROFIT_ATR_MULT"]) <= 0:
            return False
        if float(values["BAND_STOP_MIN_PROFILE_RANGE_ATR_MULT"]) < 0:
            return False
        ...
        if float(values["BAND_STOP_PROFILE_BUFFER_POINTS"]) < 0:
            return False
        if float(values["BAND_STOP_MIN_DISTANCE_ATR_MULT"]) <= 0:
            return False
        if float(values["BAND_STOP_MAX_DISTANCE_ATR_MULT"]) <= float(values["BAND_STOP_MIN_DISTANCE_ATR_MULT"]):
            return False
...
def parameter_label(values: dict[str, int | float]) -> str:
    return (
        ...
        f"alltpatr{_format_value(values['ALL_STOP_MODES_TAKE_PROFIT_ATR_MULT'])}_"
        f"bandrangeatr{_format_value(values['BAND_STOP_MIN_PROFILE_RANGE_ATR_MULT'])}_"
        ...
        f"bandstopatr{_format_value(values['BAND_STOP_MIN_DISTANCE_ATR_MULT'])}-{_format_value(values['BAND_STOP_MAX_DISTANCE_ATR_MULT'])}"
    )
